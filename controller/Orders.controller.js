...
const generatePdfReceipt = async (req, res) => {
  ...
  try {
    ...
    doc.fontSize(14).text(`City: ${selectedAddress.city}, ${selectedAddress.country} - ${selectedAddress.pincode}`);
    doc.moveDown();

    doc.fontSize(18).text('Order Details', { underline: true });
    doc.fontSize(14).text(`Order ID: ${orderSummary.orderId}`);
    doc.fontSize(14).text(`Date: ${new Date(orderDate).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    })}`);
    ...
    orderSummary.products.forEach(item => {
      doc.fontSize(14).text(`${item.product.title} (x${item.quantity}) - Rs. ${item.price.toFixed(2)}`);
      doc.moveDown();
    });

    doc.fontSize(16).text(`Total Amount: Rs. ${orderSummary.totalAmount.toFixed(2)}`, { align: 'right' });
    doc.moveDown();
    ...
    writeStream.on('finish', () => {
      res.status(200).json({ fileUrl: `/receipts/${fileName}` });
    });
...
const updateOrderDeliveryStatus = async (req, res) => {
  try {
    ...
    const updatedOrder = await Orders.findByIdAndUpdate(orderId, { orderStatus: status }, { new: true });
...
module.exports = {generatePdfReceipt, addOrder, getOrdersByUser, createPaymentIntent, updateOrderStatus,getAllOrders, updateOrderDeliveryStatus, deleteOrder };
